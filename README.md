...
print(f.read())
```
گاهی نیاز نداریم همه فایل رو بخونیم. مثلا اگر بخواهیم فق 20 کاراکتر اول فایل رو چک کنیم این کد رو مینویسیم:
```python
f = open("database.txt", "r")
print(f.read(20)) 
```
...
```python
# باز کردن فایلی جدید و اضافه کردن یک خط نوشته در انتهای آن
f = open("database2.txt", "a")
f.write("This is a line appended to the end.")
f.close()

# باز کردن فایل خطوط بالاتر و خواندن خط نوشته شده
f = open("database2.txt", "r")
print(f.read()) 
```
حالت `write` یعنی پاک کردن اطلاعات قبلی و نوشتن از اول:
```python
# باز کردن فایلی جدید و نوشتن روی آن با پاک کردن اطلاعات قبلی
